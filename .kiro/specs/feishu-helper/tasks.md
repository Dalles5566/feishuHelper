...
- [x] 17.3 完整工作流串联（严格按设计文档状态机）
  - [x] 17.3.1 注册 advance_task 工具：通用状态推进（替代 complete_task）
    - 接受 task_id + event 参数
    - event: assigned / confirmed / dev_complete / qa_passed / qa_failed_impl / qa_failed_req
    - 内部调状态机 validateTransition，非法转换直接拒绝
    - 智能跳步：confirmed 从 Created 自动走 Assigned → InDevelopment；qa_failed 从 QAPending 自动走 QAFailed → InDevelopment/Created；qa_passed 自动走 QAPassed → Completed
  - [x] 17.3.2 注册 verify_code 工具：AI 验证代码是否符合需求
    - 调 CodeVerifier.verify()，生成验证报告（仅报告，不推进状态）
    - 可选 code_changes 参数（有代码就分析，没有就基于需求生成参考报告）
    - 验证报告存入 verification_reports 表
    - 移除 CodeVerifier 内部的 advanceTaskWorkflow 调用
  - [x] 17.3.3 注册 generate_test_doc 工具：生成测试文档给 QA
    - 调 DocGenerator.generateTestDocument()
    - 测试文档存入 documents 表 + 作为附件上传到飞书任务
  - [x] 17.3.4 注册 submit_qa_feedback 工具：QA 提交测试结果
    - 一站式：存 QA 反馈到 qa_feedbacks 表 + 自动推进状态
    - 通过 → QAPassed → Completed；失败（实现错误）→ QAFailed → InDevelopment；失败（需求错误）→ QAFailed → Created
    - 内置状态检查：只有 QAPending 状态才能提交 QA 反馈
  - [x] 17.3.5 实现 syncDescriptionToFeishu 统一方法
    - 每次有事件发生时自动追加历史到飞书描述
